...
module Wirb
  SCHEMA = { :comma => [:green],
             ...
             :object_line_number => [:yellow, :underline],
  ...
  def self.paint(token, *options)
    options.map { |e| "\e[#{CODES[e] || 0}m" }.join + token + "\e[0m"
  end

  def self.colorize_result(string, _deprecated = nil)
    check = ""
    colorful = ""
    begin
